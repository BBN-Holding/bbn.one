...
import { createCachedLoader, createIndexPaginationLoader } from "webgen/extended/mod.ts";
    ...
    categories: [
        {
            ...
            children: state.$servers.map((servers) =>
                servers.length == 0 ? [placeholder("Oh soo empty!", "You servers will be here when you create some!")] : servers.map((server) =>
                    <RenderItem> {
                        id: server._id,
                        title: server.$name,
                        replacement: Grid(
        ...
        },
        {
            ...
            title: "Resources",
            children: [
                state.$meta.map((meta) => meta ? profileView() : LoadingSpinner()).asRefComponent(),
            ],
        },
    ],
    children: [
                        ...
                        List(
                            ...
                            (addon) =>
                                Entry({
                                    ...
                                    .addPrefix(
                                        Cache(`${addon.slug}:${addon.icon_url}`, undefined, () =>
                                    ...
                                    .addSuffix(
                                        Grid(
                                            ...
                                            Cache<ModrinthDownload | undefined>(addon.slug, () => addon.download, (type, val) => {
                                                ...
                                                return installedAddons.map((it) => !it.find((it) => it.projectId === addon.project_id)).map((installed) =>
                                                    installed
